...
export const inventoryCategories = pgTable(
  ...
  (table) => ({
    locationIdx: index("inventory_categories_location_id_idx").on(table.locationId),
    locationNameUnique: unique("inventory_categories_location_name_unique").on(table.locationId, table.name),
  })
);
...
export const inventoryMovements = pgTable(
  ...
  (table) => ({
    ...
    locationIdx: index("inventory_movements_location_id_idx").on(table.locationId),
  })
);

export const inventoryCategoriesRelations = relations(inventoryCategories, ({ one, many }) => ({
  location: one(locations, { fields: [inventoryCategories.locationId], references: [locations.id] }),
...
export const inventoryMovementsRelations = relations(inventoryMovements, ({ one }) => ({
  ...
  location: one(locations, { fields: [inventoryMovements.locationId], references: [locations.id] }),
  appointment: one(appointments, { fields: [inventoryMovements.appointmentId], references: [appointments.id] }),
