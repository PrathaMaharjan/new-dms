...
type BillingPatientRow = {
  ...
  lastTreatmentName: string | null; // ADDED
  lastTreatmentCostCents: number | null; // ADDED
  chargedCents: number;
  paidCents: number;
  balanceCents: number;
};
...
export default function BillingPage() {
  ...
  const [balanceFilter, setBalanceFilter] = useState<"All" | "Due" | "Settled">(
    "All",
  );
  ...
  const [entryTargetPatientId, setEntryTargetPatientId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    async function loadStaffLocation() {
      try {
        ...
        const [outletsRes, servicesRes, treatmentsRes, patientsRes] =
          await Promise.all([
        ...
        if (
          outletsRes?.data?.success &&
          Array.isArray(outletsRes.data.data.locations) &&
          outletsRes.data.data.locations.length > 0
        ) {
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const { data: responseBody } = await axios.get(
        `/api/billing/stats?locationId=${locationId}`,
      );
  ...
  const loadPatients = useCallback(async () => {
    ...
    try {
      ...
      if (responseBody?.success && Array.isArray(responseBody.data.patients)) {
      ...
      console.error("Failed to load billing patients:", err);
      setRows([]);
      setTotal(0);
    } finally {
    ...
    return [
      {
        icon: Receipt,
        label: "Total Charged",
        value: `NPR ${centsToDisplay(stats.totalChargedCents)}`,
      },
      {
        icon: Wallet,
      ...
      },
      {
        ...
        value: String(stats.patientsWithDuesCount),
      },
    ];
  }, [stats]);
  ...
  function update<K extends keyof EntryFormState>(
    ...
    setEntryForm((prev) => {
      ...
      if (key === "type" && value === "payment" && entryTargetPatientId) {
        const targetRow = rows.find(
          (r) => r.patientId === entryTargetPatientId,
        );
        if (targetRow && targetRow.balanceCents > 0) {
          next.amount = (targetRow.balanceCents / 100).toFixed(2);
        }
      }
  ...
  async function handleAddEntry(e: React.FormEvent) {
    ...
    try {
      ...
      if (!responseBody?.success) {
        setEntryError(
          responseBody?.error ?? "Something went wrong adding this entry.",
        );
        return;
      }
      ...
      if (axios.isAxiosError(err)) {
        setEntryError(
          err.response?.data?.error ??
            "Something went wrong adding this entry.",
        );
      } else {
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  onChange={(e) => {
                    setBalanceFilter(
                      e.target.value as "All" | "Due" | "Settled",
                    );
                    setCurrentPage(1);
                  }}
              ...
              <tbody className="divide-y divide-slate-900/5 bg-white">
                {rows.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-[0.85rem]">
                        {r.lastTreatmentName ? (
                          <div>
                            <p className="font-medium text-slate-800">
                              {r.lastTreatmentName}
                            </p>
                            {r.lastTreatmentCostCents !== null && (
                              <p className="mt-0.5 flex items-center gap-1 text-[0.78rem] font-semibold text-[#3f6274]">
                                <Receipt className="h-3 w-3" strokeWidth={2} />
                                NPR {centsToDisplay(r.lastTreatmentCostCents)}
                              </p>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="px-5 py-4 text-[0.85rem] text-slate-700">
                        ...
                        {centsToDisplay(
                          r.chargedCents > 0
                            ? r.chargedCents
                            : (r.lastTreatmentCostCents ?? 0),
                        )}
                      </td>
                      <td className="px-5 py-4 text-[0.85rem] text-slate-700">
                        NPR {centsToDisplay(r.paidCents)}
                      </td>
                      <td className="px-5 py-4">
                        <span
                ...
                {!loadingTable && rows.length === 0 && (
                  <tr>
                    <td
                      colSpan={8}
                      className="bg-white py-16 text-center text-slate-500"
                    >
                      {" "}
                      {/* CHANGED - was 7 */}
                ...
                {loadingTable && (
                  <tr>
                    <td
                      colSpan={8}
                      className="bg-white py-16 text-center text-slate-400"
                    >
                      {" "}
                      {/* CHANGED - was 7 */}
                      Loading...
          ...
          {total > 0 && (
            <div className="mt-4 flex items-center justify-between border-t border-slate-100 px-1 pt-4 text-xs">
              <span className="text-[0.7rem] font-medium text-slate-500">
                ...
                of <strong className="text-slate-800">{total}</strong> patients
              </span>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  {ledgerSummary && (
                    ...
                    >
                      NPR{" "}
                      {centsToDisplay(Math.abs(ledgerSummary.balanceDueCents))}
                      {ledgerSummary.balanceDueCents > 0
                        ? " due"
                        : ledgerSummary.balanceDueCents < 0
                          ? " credit"
                ...
                {loadingLedger ? (
                  ...
                  <div className="mt-4 space-y-3">
                    {[...ledgerEntries]
                      ...
                      .map((entry) => {
                        const TypeIcon =
                          ENTRY_TYPE_ICONS[entry.type as EntryType];
                        const MethodIcon = entry.paymentMethod
                          ? METHOD_ICONS[entry.paymentMethod]
                          : null;
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <span
                                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${ENTRY_TYPE_COLORS[entry.type as EntryType]}`}
                              >
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  <span
                                    className={`inline-flex items-center rounded-full px-2 py-0.5 text-[0.7rem] font-medium ${ENTRY_TYPE_COLORS[entry.type as EntryType]}`}
                                  >
                                  ...
                                  {entry.paymentMethod && (
                                    <span className="inline-flex items-center gap-1 text-[0.75rem] text-slate-500">
                                      ...
                                      {PAYMENT_METHOD_LABELS[
                                        entry.paymentMethod as (typeof PAYMENT_METHODS)[number]
                                      ] ?? entry.paymentMethod}
                                    </span>
                                  )}
                                </div>
                            ...
                            <div className="flex shrink-0 flex-col items-end gap-2">
                              <p
                                className={`text-[0.9rem] font-semibold ${entry.type === "payment" || isNegativeAdjustment ? "text-emerald-600" : "text-slate-800"}`}
                              >
                                {entry.amountCents < 0 ? "−" : "+"}
                                NPR{" "}
                                {centsToDisplay(Math.abs(entry.amountCents))}
                              </p>
                              {entry.type === "payment" && (
                                <button
                                  type="button"
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <p className="mt-1 text-[0.8rem] text-slate-500">
              {entryTargetPatientId === selectedPatientId
                ? selectedPatientName
                : rows.find((r) => r.patientId === entryTargetPatientId)
                    ?.patientName}
            </p>
            ...
            <form onSubmit={handleAddEntry} className="mt-4 space-y-4">
              ...
              <label className="block">
                ...
                {/* ADDED - explains WHY the amount is pre-filled, so it doesn't look like a mysterious default */}
                {entryForm.type === "payment" &&
                  (() => {
                    const targetRow = rows.find(
                      (r) => r.patientId === entryTargetPatientId,
                    );
                    return targetRow && targetRow.balanceCents > 0 ? (
                      <p className="mt-1.5 text-[0.75rem] text-slate-400">
                        Pre-filled with the full outstanding balance — edit for
                        a partial payment.
                      </p>
                    ) : null;
                  })()}
            ...
            <div id="receipt-print-area" className="overflow-y-auto px-6 py-6">
              {!receiptContext ? (
                <div className="py-10 text-center text-[0.85rem] text-slate-500">
                  Couldn't find this payment entry. It may have been removed —
                  close this and try again from the ledger.
                </div>
              ) : (
                <>
                  <div className="flex items-start justify-between">
                  ...
                  <div className="mt-4 space-y-2 text-[0.85rem]">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-slate-800">
                        {receiptContext.patientName}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500">Phone</span>
                      <span className="font-medium text-slate-800">
                        {receiptContext.patientPhone || "—"}
                      </span>
                    </div>
                  ...
                  <div className="mt-5 rounded-xl bg-slate-50 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[1.15rem] font-semibold text-slate-900">
                        NPR{" "}
                        {centsToDisplay(
                          Math.abs(receiptContext.entry.amountCents),
                        )}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex items-center justify-between text-[0.8rem]">
                    ...
                    {(() => {
                      ...
                      return (
                        <span
                          className={`font-medium ${due ? "text-rose-600" : "text-emerald-600"}`}
                        >
                          NPR {centsToDisplay(Math.abs(bal))}{" "}
                          {due ? "due" : "settled"}
                        </span>
                      );
                    })()}
