...
const SEED_OUTLETS: Outlet[] = [
    ...
    { id: "outlet-3", name: "Baneshwor Branch" },
];
...
const SEED_EXPENSES: ExpenseItem[] = [
    ...
    {
        expenseId: "exp-2",
        description: "Kitchen exhaust repair",
        category: "Maintenance",
        categoryId: "cat-2",
        amount: 4200,
        date: "2026-08-03",
        notes: "Vendor: Sharma Electricals",
        source: "manual",
        editable: true,
        outletId: "outlet-1",
    },
    {
        expenseId: "exp-3",
        description: "Restock: Basmati Rice",
        category: "Supplies",
        categoryId: "cat-3",
        amount: 12300,
        date: "2026-08-05",
    ...
    {
        expenseId: "exp-6",
        description: "Restock: Cooking Oil",
        category: "Supplies",
        categoryId: "cat-3",
        amount: 5400,
        date: "2026-08-09",
...
export default function ExpensesPage() {
    ...
    const [activeOutletId, setActiveOutletId] = useState<string>(SEED_OUTLETS[0].id);
    ...
    function addNewCategory() {
        ...
        if (existing) {
            ...
            const newCat: ExpenseCategory = { id: `cat-${Date.now()}`, name: trimmed };
    ...
    const outletExpenses = expenses.filter((e) => e.outletId === activeOutletId);

    const filteredExpenses = outletExpenses.filter((expense) => {
        const matchesSearch =
            !search.trim() ||
        ...
        const matchesCategory =
            categoryFilter === "All" ||
            (categoryFilter === "inventory" ? expense.source === "inventory" : expense.categoryId === categoryFilter);
    ...
    const stats = {
        totalExpenses: outletExpenses.length,
        totalSpend: outletExpenses.reduce((sum, e) => sum + e.amount, 0),
        fromInventory: outletExpenses.filter((e) => e.source === "inventory").length,
        manualCount: outletExpenses.filter((e) => e.source === "manual").length,
    };
    ...
    function handleSaveExpense() {
        ...
        if (editingExpense) {
            setExpenses((prev) =>
                prev.map((e) =>
                    e.expenseId === editingExpense.expenseId
                        ? {
                            ...e,
    ...
    return (
        <div className="relative min-h-screen bg-slate-50" onClick={() => outletDropdownOpen && setOutletDropdownOpen(false)}>
            {/* Sticky Header */}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative" onClick={(e) => e.stopPropagation()}>
                            ...
                            >
                                ...
                                <span className="max-w-[140px] truncate">{activeOutlet?.name ?? "Select Outlet"}</span>
                                <ChevronDown
                            ...
                            {outletDropdownOpen && (
                                <div className="absolute right-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-2xl border border-slate-900/5 bg-white shadow-xl">
                                    ...
                                    <div className="py-1">
                                        {outlets.map((outlet) => (
                                            <button
                                                ...
                                                className={`flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-[0.85rem] transition-colors ${activeOutletId === outlet.id
                                                        ? "bg-[#7da3b3]/10 font-semibold text-[#3f6274]"
                                                        : "text-slate-700 hover:bg-slate-50"
                                                    }`}
                                            >
                                                <span
                                                    className={`h-2 w-2 shrink-0 rounded-full ${activeOutletId === outlet.id ? "bg-[#7da3b3]" : "bg-slate-200"
                                                        }`}
                                                />
                                                {outlet.name}
                                            </button>
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 rounded-full bg-[#3f6274] px-5 py-2.5 text-[0.85rem] font-medium text-white shadow-sm transition-colors hover:bg-[#345263]"
                        >
                    ...
                    >
                        ...
                        <div className="flex flex-col-reverse gap-2 border-t border-slate-900/5 bg-slate-50/50 p-6 sm:flex-row sm:justify-end sm:gap-3">
                            ...
                            <button
                                ...
                                className="flex w-full items-center justify-center gap-2 rounded-full bg-[#3f6274] px-5 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-[#345263] sm:w-auto"
                            >
