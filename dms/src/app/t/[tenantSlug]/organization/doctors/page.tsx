...
export default function DoctorsPage() {
  ...
  async function loadData() {
    try {
      ...
      const activeLoc = outletFilter && outletFilter !== "all" ? outletFilter : undefined;
      const [servicesRes, outletsRes, treatmentsRes, res, apptsRes] = await Promise.all([
        axios.get("/api/services").catch(() => null),
        axios.get("/api/outlets").catch(() => null),
        axios.get("/api/treatment", {
          params: activeLoc ? { locationId: activeLoc, limit: 100 } : { limit: 100 },
        }).catch(() => null),
      ...
      if (outletsRes?.data?.success && outletsRes.data.data?.locations) {
        ...
        if (mappedOutlets.length > 0 && !outletFilter) {
          setOutletFilter(mappedOutlets[0].id);
        }
      }
                    ...
                    <>
                      ...
                      <div className="mt-6 border-t border-slate-900/5 pt-5">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="rounded-full bg-[#7da3b3]/15 px-2.5 py-0.5 text-xs font-semibold text-[#345263]">
                            {(() => {
                              const targetLoc = outletFilter && outletFilter !== "all" ? outletFilter : undefined;
                              const count = targetLoc
                                ? (selectedDoctor.treatments || []).filter((t) => t.locationId === targetLoc).length
                        ...
                        <div className="mt-3">
                          {selectedDoctor.treatments && selectedDoctor.treatments.length > 0 ? (
                            <div className="space-y-4">
                              {(() => {
                                const targetLoc = outletFilter && outletFilter !== "all" ? outletFilter : undefined;
                                const filteredDoctorTreatments = targetLoc
                                  ? (selectedDoctor.treatments || []).filter((t) => t.locationId === targetLoc)
                                ...
                                if (filteredDoctorTreatments.length === 0) {
                                  return (
                                    <p className="text-xs text-slate-500 italic">
                                      {targetLoc
                                        ? "No treatments assigned to this doctor for the selected outlet."
