import { createExpenseCategory, ExpenseErrorCode, getExpenseCategories } from "@/controller/expenses/controller";
...
export async function POST(request: NextRequest) {
  ...
  const result = await createExpenseCategory(body);
...
export async function GET(request:NextRequest) {
  ...
  const result = await getExpenseCategories(locationId);
  if (!result.success) return NextResponse.json({ success: false, error: result.error }, { status: STATUS_BY_CODE[result.code] });
