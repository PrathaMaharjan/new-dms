import { deleteTreatment, TreatmentErrorCode, updateTreatment } from "@/controller/treatments/controller";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await updateTreatment(id, body);
  ...
  return NextResponse.json({ success: true, statusCode: 200, data: { treatment: result.treatment } });
}
