import { getFrontDeskAppointmentTrend } from "@/controller/frontdesk-dashboard/controller";
import { NextRequest, NextResponse } from "next/server";

export async function GET(request: NextRequest) {
  const locationId = request.nextUrl.searchParams.get("locationId");
  const range = (request.nextUrl.searchParams.get("range") || "7days") as "7days" | "30days" | "1year" | "7d" | "1m" | "1y";

  if (!locationId) {
    return NextResponse.json({ success: false, error: "locationId is required" }, { status: 400 });
  }
  const result = await getFrontDeskAppointmentTrend(locationId, range);
  if (!result.success) {
    return NextResponse.json({ success: false, error: result.error }, { status: result.code === "UNAUTHORIZED" ? 401 : 500 });
  }
  return NextResponse.json({ success: true, statusCode: 200, data: { days: result.days, trend: result.trend } });
}