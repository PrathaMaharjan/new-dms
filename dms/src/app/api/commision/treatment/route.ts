...
import { getCommissionRateMatrix, setCommissionRate } from "@/controller/commissions/treatment/controller";
...
export async function GET(request: NextRequest) {
  ...
  const result = await getCommissionRateMatrix(locationId);
  if (!result.success) return NextResponse.json({ success: false, error: result.error }, { status: STATUS_BY_CODE[result.code] });
  return NextResponse.json({ success: true, statusCode: 200, data: { matrix: result.matrix } });
}

export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => null);
  const result = await setCommissionRate(body);
  if (!result.success) return NextResponse.json({ success: false, error: result.error }, { status: STATUS_BY_CODE[result.code] });
