import { createService, getServices, ServiceErrorCode } from "@/controller/services/services";
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ success: true, statusCode: 201, data: { service: result.service } }, { status: 201 });
}
...
export async function GET(request: NextRequest) {
  ...
  const result = await getServices(locationId, { limit, offset });

  if (!result.success) {
    const status = STATUS_BY_CODE[result.code];
    return NextResponse.json({ success: false, statusCode: status, error: result.error, code: result.code }, { status });
  }
  return NextResponse.json({ success: true, statusCode: 200, data: { services: result.services, pagination: result.pagination } });
}
