...
import { platformAdminLoginController, SUPERADMIN_ACCESS_TOKEN_MAX_AGE_SECONDS } from "@/controller/superadmin/auth/controller";
import { NextRequest, NextResponse } from "next/server";

export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => null);
  const result = await platformAdminLoginController(body);
